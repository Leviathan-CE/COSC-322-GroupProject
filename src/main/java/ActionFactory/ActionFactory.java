...
public class ActionFactory {
	...
	public static ArrayList<Node> getLegalMoves(Node parent, int player) {
	    // Initialize an empty ArrayList to store the legal moves.
	    ArrayList<Node> legalMoves = new ArrayList<>();
	    int[][] gameboard = parent.getCurBoard();
	            ...
	            if (gameboard[i][j] == player) {
	                ...
	                for (int dx = -1; dx <= 1; dx++) {
	                    for (int dy = -1; dy <= 1; dy++) {
	                        // Skip over the case where both dx and dy are 0, which corresponds to not moving the piece.
	                        if (dx == 0 && dy == 0) {
	                            continue;
	                        }
	                            ...
	                            if (gameboard[x][y] == 0) {
	                                ...
	                                for (int ax = -1; ax <= 1; ax++) {
	                                    for (int ay = -1; ay <= 1; ay++) {
	                                        // Initialize temporary variables to store the current position of the arrow.
	                                        int bx = x;
	                                        int by = y;
	                                        ...
	                                        while (bx + ax >= 0 && bx + ax < 10 && by + ay >= 0 && by + ay < 10) {
	                                            ...
	                                            Node game = new Node(arrowGameboard, parent);
	                                ...
	                                // If the current cell is occupied, the move is not legal and we move on to the next direction.
	                                break;
	                            }
	                        }
	                    }
