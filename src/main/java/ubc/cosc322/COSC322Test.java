...
//				}
				...
				for (Node n : chioces) {
					n.C = Math.random() * 5;
					chessBoard[1].addChild(n);
				}
				//pick the highest and deepest node to pick for our move
				OurMove = MonteTreeSearch.Search(chessBoard[1]);
				OurMove.updateQueenPoses();

				// exctract our move from the new game state using the positions of the queens
				// and arrwos from
